...
async function load() {
  loading.value = true;
  try {
    const res: any = await wmsApi.reconcileList({ page: page.value, size: size.value });
...
async function onCreate() {
  ...
  try {
    const res: any = await wmsApi.reconcileCreate({
...
async function onOpenDetail(no: string) {
  try {
    const res: any = await wmsApi.reconcileDetail(no);
...
async function onConfirm(no: string) {
  try {
    ...
    await wmsApi.reconcileConfirm(no);
...
<template>
  <div>
    <div class="page-head">
      <div>
        <h2 class="page-title">OMS 对账</h2>
        <p class="page-desc">WMS /reconcile · iter-24 P1-2 · 比较 WMS.quantity-locked vs OMS.available · 仅记录不自动修复</p>
      </div>
      <div>
    ...
    <el-table :data="list" v-loading="loading" stripe border style="margin-top:16px;">
      <el-table-column prop="reconcile_no" label="单号" width="200" />
    ...
    <!-- 对账详情 -->
    <el-dialog v-model="detailVisible" :title="`对账详情 ${detail?.reconcile_no || ''}`" width="1100px" top="5vh">
      ...
      <el-table :data="detail?.details || []" border stripe style="margin-top:12px;" :row-class-name="({row}) => row.match ? '' : 'row-diff'">
        ...
        <el-table-column label="OMS reserved" width="120" prop="oms_reserved" />
        <el-table-column label="差异 (WMS-OMS)" width="140">
          <template #default="{ row }">
