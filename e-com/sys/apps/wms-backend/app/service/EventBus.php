...
/**
 * Redis Stream 事件总线（WMS 副本，与 OMS 同源）
 *
 * - 用独立 Redis 连接（db=0），与 OMS 共享 Stream
 * - publish：XADD + MAXLEN 10000
 * - consume：XGROUP CREATE MKSTREAM 兜底 + XREADGROUP BLOCK
 * - 失败 >= 3 次 写 dead_letter 表 + ACK
 *
 * 消息格式：{event_id, trace_id, ts, payload(JSON)}
 */
class EventBus
{
