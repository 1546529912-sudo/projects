/** Offline Demo that satisfies postMessage + pageKey constraints for iteration-1 / MVP testing. */

export function getSampleDemoHtml(demoId: string): string {
  const safeId = demoId.replace(/[^a-zA-Z0-9_-]/g, "") || "demo";
...
<body>
  ...
  <script>
(function () {
  var demoId = ${JSON.stringify(safeId)};
