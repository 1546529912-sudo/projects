...
- AI 应尽量**一次性生成完整项目框架**，而不是等待用户一轮轮追问后再补缺口
- 若已能合理推断下一步所需基础文档，应直接一并生成
- 只有在涉及高风险技术选型、需求冲突或缺少关键输入时，才应暂停等待确认
...
- 如果用户只提供功能列表，AI 仍应一次性先搭出完整协作骨架，而不是只输出一份需求文档
...
10. 技术选型必须考虑团队部署成本；如果没有更强的反向约束，默认优先推荐 **PHP 方案**，并在架构文档里明确写出“为什么推荐 PHP”，尤其要说明：
    - 团队部署更容易
    - 运维和交接成本更低
    - 更适合单体工作台类 MVP
11. 所有角色都要有自己的 skill 包，skill 中至少包含：
    - read first
    - responsibilities
    - workflow
    - required outputs
    - guardrails
    - blocking / escalation
12. 如果仓库中已经存在某个前端或后端原型，不要默认把原型技术直接当成最终生产方案；需要在架构文档中单独判断“原型技术”与“正式技术方案”的关系
13. 如果我只提供产品功能列表，没有完整需求文档或 MVP 划分，你要自动推导：
    - 初版需求文档
    - 初版 MVP 范围
    - 初版明确不做
    - 初版第一轮计划
...
14. 第一轮 runbook 必须明确：
    ...
    - 阶段切换条件
    - 风险与待确认项
15. 初始化项目时，除了模板文件外，还要生成：
    - `outputs/` 目录结构
    - 第一份真实 `progress` 回写样例文档
    - 第一批真实可勾选的 `progress` 阶段回写案例
16. 如果后续出现阻塞，阻塞不仅要写入 blocker 文档，也要同步回写到 `progress.md` 的“当前问题”区
17. 第一轮至少应同步生成：
    - 产品产物
    - 设计产物
    - 架构产物
    ...
    - 设计阶段真实回写样例
    - 架构阶段真实回写样例
18. 若推荐 PHP 方案，架构文档中至少要明确：
    - 推荐技术栈，如 Laravel + MySQL + Alpine.js / 原生 JS
    - 为什么它比当前原型技术更适合正式部署
    - 当前原型代码是保留参考还是后续废弃
19. 生成的每份文档不要只给空壳，必须写出实际内容、默认规则和最小可执行结构
...
- `outputs/orchestration/iteration-1-design-progress-update.md`
- `outputs/orchestration/iteration-1-architecture-progress-update.md`
- `outputs/orchestration/progress-update-template.md`
- `outputs/orchestration/blocker-escalation-template.md`
...
- 推荐开发顺序
- 核心约束
- 若已有现成原型或团队技术偏好，应写明“当前原型”与“正式方案”的关系
- 若部署便利性是强约束，应在需求文档中写明“技术方向优先考虑 PHP / 部署友好方案”
...
- 交接最小模板
- 回写规则
- 默认主控 Agent 负责整段推进，不应让用户逐阶段手动追问
...
11. 写第一轮设计产物
12. 写第一轮架构产物
13. 写第一份产品阶段真实 `progress-update` 样例
14. 写第一份设计阶段真实 `progress-update` 样例
15. 写第一份架构阶段真实 `progress-update` 样例
16. 将首批证据充分的阶段真实回写到 `progress.md`
17. 由主控 Agent 推进后续开发、测试
...
- 如果已经能推断产品、设计、架构三个前置阶段，就应一次性补齐，不要让用户逐个索要
- 如果已有原型代码或现成技术栈，必须显式判断它是“正式方案”还是“原型参考”
